...
LevelDataManager::LevelDataManager():
	level_datas_(make_unique<LevelData>())
{}

LevelDataManager::~LevelDataManager()
{}

void LevelDataManager::LoadLevelByJson(std::string path)
{
	// ファイルストリーム
	std::ifstream file;
	...
	for (json &object : deserialized["object"])
	{
		...
		{
			// 要素追加
			level_datas_->object_data.emplace_back(LevelDataManager::ObjectData{});

			// 参照を得る
			LevelDataManager::ObjectData &obj_data = level_datas_->object_data.back();
