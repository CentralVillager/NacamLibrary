...
void TemporaryScene::Update()
{
	...
	{
		if (KeyboardInput::PushKey(DIK_UP)) { camera_->MoveCameraTrack({ 0, 1.0f, 0 }); }
		else if (KeyboardInput::PushKey(DIK_DOWN)) { camera_->MoveCameraTrack({ 0, -1.0f, 0 }); }
		if (KeyboardInput::PushKey(DIK_RIGHT)) { camera_->MoveCameraTrack({ 1.0f, 0, 0 }); }
		else if (KeyboardInput::PushKey(DIK_LEFT)) { camera_->MoveCameraTrack({ -1.0f, 0, 0 }); }
	}
