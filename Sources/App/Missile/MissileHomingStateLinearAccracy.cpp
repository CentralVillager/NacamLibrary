#include "MissileHomingStateLinearAccracy.h"
#include "Missile.h"
...
void MissileHomingStateLinearAccracy::HomingTarget(Missile &missile)
{
	MissileParam *mi_param = &missile.GetMissileParam();
	mi_param->accuracy_type = HomingAccuracy::Linear;

	static uint32_t homing_delay = 300;
	...
	{
		// 正規化
		XMStoreFloat3(&mi_param->vel, XMVector3Normalize(XMLoadFloat3(&mi_param->vel)));
	}
